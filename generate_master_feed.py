...
# ── Keywords to exclude from both feeds (gift cards, vouchers) ───────────
EXCLUDED_KEYWORDS = [
    "gift card", "gift voucher", "voucher", "e-voucher", "egift",
...
def product_id(p):
    """Always use the Ecwid numeric product ID as g:id."""
    return str(p.get("id", ""))
...
def build_shopping(products):
    ...
    added = skipped = sales = excluded = 0

    for p in products:
        # Skip gift cards / vouchers
        if is_excluded(p):
            print(f"  EXCLUDED (gift card/voucher): {p.get('name','')}")
            excluded += 1
            continue
        ...
        if gtin_ok(gtin_val):
            g(item, "gtin", gtin_val)
        elif not brand and not sku:
        ...
        if wtstr and wtstr not in ("0.0 g", "0 g"):
            g(item, "shipping_weight", wtstr)
    ...
    return rss, added, skipped, sales, excluded
...
def main():
    ...
    if not TOKEN:
        print("ERROR: ECWID_TOKEN not set"); raise SystemExit(1)
    ...
    rss, added, skipped, sales, excl = build_shopping(products)
    print(f"Added:{added}  Sales:{sales}  Skipped:{skipped}  Excluded:{excl}")

    print("Building local inventory feed...")
    lrss, ladded, lexcl = build_local(products)
    print(f"Local:{ladded}  Excluded:{lexcl}")
